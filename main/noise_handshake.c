...
static void handle_noise_payload(uint16_t conn_handle, const noise_event_t *evt, noise_session_t *session, const uint8_t *decrypted, size_t decrypted_len)
{
    ...
    /* Bridge uplink: forward a received mesh private message to the wt
     * channel, preserving the sender's identity (#<nickname>: content). */
    if (session && content_len > 0) {
        const char *sender_name = "";
        size_t idx = (size_t)(session - s_sessions);
        if (idx < NOISE_MAX_SESSIONS && s_identities[idx].valid) {
            sender_name = s_identities[idx].nickname;
        }
        wifi_bridge_send_up(session->peer_id, sender_name, WIFI_BRIDGE_WT, content);
    }
...
/* 把一条 wt 频道的消息作为私信注入本地 mesh：向每个已建立会话的直接 peer
 * 发送。正文格式 "#<发送者>: <内容>"，保留原始发送者信息。 */
int noise_bridge_forward_to_mesh(const char *sender_name, const char *wt, const char *content)
{
    char body[WIFI_BRIDGE_CONTENT_MAX + 32];
    if (!content) {
        return 0;
    }
    int n = snprintf(body, sizeof(body), "[%s] #%s: %s",
                     wt ? wt : "wt", sender_name ? sender_name : "?", content);
    if (n < 0) {
        n = 0;
    }
    ...
    uint8_t payload[2 + sizeof(message_id) + 2 + sizeof(body)];
    size_t o = 0;
    payload[o++] = 0x00;
    payload[o++] = (uint8_t)id_len;
    memcpy(payload + o, message_id, id_len);
    o += id_len;
    payload[o++] = 0x01;
    payload[o++] = (uint8_t)n;
    memcpy(payload + o, body, n);
    o += n;

    int sent = 0;
    for (size_t i = 0; i < NOISE_MAX_SESSIONS; i++) {
        noise_session_t *s = &s_sessions[i];
        if (s->in_use && s->established) {
            if (noise_send_encrypted(s->conn_handle, BITCHAT_NOISE_PAYLOAD_PRIVATE_MESSAGE, payload, o)) {
                sent++;
            }
        }
    }
    ESP_LOGI(TAG, "bridge forward to mesh: %d peer(s) sent", sent);
    return sent;
}
